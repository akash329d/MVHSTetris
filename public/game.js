...
class TetrisGame {
	...
	handlePower(data){
	    switch(data[0]){
            ...
            case "E":
                for(var s = 0; s < 20; s ++) {
                    var copiedArray = JSON.parse(JSON.stringify(this.boardStatic[s]));
                    for(var f = 0; f < 10; f ++) {
                        var randNum = Math.floor(Math.random() * copiedArray.length);
                        this.boardStatic[s][f] = copiedArray[randNum];
                        copiedArray.splice(randNum, 1);
                    }
                }
                break;
            case "R":
